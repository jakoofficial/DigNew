...
func _show(a: ArtifactRes) -> void:
	...
	tween.parallel()
	tween.tween_property(artifact_info, "scale", Vector2(1,1), 0.5)
	...
	tween.parallel()
	tween.tween_property(artifact_info, "scale", Vector2(0,0), 0.5).set_delay(2.0)
	tween.parallel()
